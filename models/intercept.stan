...
transformed parameters{
    ...
    vector<lower=0,upper=1>[N] gb_temp;
    
    for (i in 1:N) {
        gb_temp[i] = inv_logit(ib + ib_vowel[vowel[i]] + ib_group[group[i]]) * collective_benefit[group[i]];
        gb_group[group[i]] = logit(gb_temp[i]) - (gb + gb_vowel[vowel[i]]);
    }
}
...
//         itheta[i] =  iA[i] + iB[i] * intensity[i];
// 
//         log_lik[i]  = binomial_logit_lpmf(k_gro | n_gro, gtheta) + binomial_logit_lpmf(k_ind | n_ind, itheta);
//     }
// 
// 
// }
